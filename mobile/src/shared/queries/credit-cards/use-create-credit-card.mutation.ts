...
export const useCreateCreditCardMutation = () => {
  ...
  const mutation = useMutation({
    mutationFn: (creditCardData: CreateCreditCardRequestParams) => createCreditCard(creditCardData),
    onSuccess: (response) => {
      Toast.success(response.message ?? "Cartão de crédito criado com sucesso!")
