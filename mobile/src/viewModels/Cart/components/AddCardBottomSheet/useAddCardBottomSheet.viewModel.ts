...
export const useAddCardBottomSheetViewModel = () => {
  ...
  const handleCreateCreditCard = handleSubmit(async ({ CVV, expirationDate: rawExpirationDate, number }) => {
    const expirationDate = formatExpirationDateFormApi(rawExpirationDate, (message) => setError("expirationDate", { message }))
